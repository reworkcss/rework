...
function rework(str, options) {
  ...
  options.str = str; // we need this for error reporting
  return new Rework(css.parse(str, options), options);
}
...
Rework.prototype.formatError = function(err) {
  // no position info, just return error as is
  if (!err.position) {
    return err;
  }

  var msg = (err.position.source || '') +
    'at line ' + err.position.start.line +
    ' column ' + err.position.start.column +
    ': ' + err.message;

  // we can provide context
  if (this.options.str && err.position.source === this.options.source) {
    var line = this.options.str.split('\n')[err.position.start.line - 1];
    msg += '\n' + line;
    msg += '\n' + new Array(err.position.start.column).join(' ') + '^';
  }

  var decorated = new Error(msg);
  decorated.position = err.position;
  decorated.originalMessage = err.message;
  return decorated;
}
